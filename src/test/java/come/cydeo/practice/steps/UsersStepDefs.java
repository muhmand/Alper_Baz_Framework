...
public class UsersStepDefs {
    ...
    public void the_user_should_see_same_number_of_user_count_in_database(String string) {
        ...
        DB_Util.runQuery("select count(status)  from users where status like'"+string+"'");
        String expectedInactiveUser = DB_Util.getFirstRowFirstColumn();
        // DB_Util.getCellValue(1,1);
        //String[] result = usersPage.NumberOfInactiveUsers.getText().split(" ");

        List<String> result = new ArrayList<>(Arrays.asList(usersPage.NumberOfInactiveUsers.getText().split(" ")));

        Assert.assertEquals(result.get(5),expectedInactiveUser);

        /// 1     10   128 15

        // Assert.assertTrue(usersPage.NumberOfInactiveUsers.getText().contains(expectedInactiveUser));
        //System.out.println(usersPage.NumberOfInactiveUsers.getText());
//        System.out.println(expectedInactiveUser);
